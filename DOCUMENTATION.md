...
| 2026-08-23 | **Nachschub-Alarm per Web Push** (`feat/reorder-push`, PR #40, 11 Commits): Vercel-Cron `17 6 * * *` → `/api/push/check-reorder` liest `coffee_stock`, rechnet das Leerdatum und schickt Web Push. Neue Tabellen `coffee_stock` (Singleton, `check id=1`) und `coffee_push_subscriptions` + `coffee_push_diag`; DB-Zugriff über die eingeschränkte Rolle `coffee_push`, die `coffees` **nicht** lesen darf. **`coffee_stock` bewusst als eigene Tabelle statt Spalte auf `coffees`** — `coffees` ist ein Sorten-Katalog (Dedup auf `roaster`+`description`, ein Nachkauf erzeugt dort keine Zeile), der Vorrat ist eine andere Entität. Vier Fehler gefunden, zwei davon älter als die Sitzung: (1) **der Service Worker installierte sich seit jeher nie** — `urlsToCache` enthielt `/static/css/main.css` + `/static/js/main.js`, die es in einem CRA-Build nicht gibt, und `cache.addAll()` ist atomar, also scheiterte `install` in `waitUntil` und `navigator.serviceWorker.ready` löste nie auf (v3: `allSettled`, `skipWaiting`, `clients.claim`); (2) der `fetch`-Handler war cache-first und hätte ab der Reparatur `index.html` dauerhaft ausgeliefert (jetzt network-first bei Navigation, Stale-Bundle-Falle); (3) die Anmeldung schrieb aus dem Browser gegen RLS statt serverseitig (jetzt `POST /api/push/subscribe` mit Host-Allowlist der Push-Dienste); (4) `clients.openWindow()` mit fremder Adresse öffnet auf Android die App statt des Ziels → Sprung zum Shop über die eigene Zwischenseite `/go.html` mit fester Zielliste. Drei Messpunkte eingebaut, die die Fehlersuche jeweils entschieden haben: Fehler-Beacon, `SW_VERSION`-Meldung beim Aktivieren, Klick-Melder. Tests: 21 API (`node --test`) + 10 Frontend, inkl. Regressionstest, der die Precache-Liste gegen das echte Build-Verzeichnis prüft. UI: Menü = Aktion, Zustand = Header-Plakette „Alarm aktiv". Erste Push-Zustellung verifiziert (`sent: 1`, vom Nutzer bestätigt). **Abends:** Bestellung **74353** ausgelöst (89,96 €, bezahlt, Versand Post.at 0,00 € — bei 89,96 € greift auch die Post-Freigrenze von 89 €); Bestand auf `bags = 5` gesetzt (laufender Beutel + 4 bestellte, `opened_at` bleibt 2026-08-05 → leer am **2026-12-21**). Zwei weitere Fehler behoben: `clients.openWindow()` mit fremder Adresse öffnet auf Android die App statt des Ziels → Sprung über die eigene Zwischenseite `/go.html` mit fester Zielliste; und der Melder hätte trotz erledigter Bestellung täglich gemahnt → neue Spalte `coffee_stock.suppress_until` (`tier: 'ordered'`), bewusst **kein dauerhaftes** Stummschalten, damit eine ausbleibende Lieferung wieder auffällt. Dabei erzeugte der Kontrollaufruf selbst einen Fehler: `buildPayload` hatte für `ordered` keinen Zweig und schickte „Leer am null (noch null Tage)" an ein echtes Gerät — behoben, Test prüft jetzt für **jeden** Zustand auf `null`/`undefined`. **Cron war nicht überprüfbar**: `notified_on` entsteht nur beim Senden, im Normalfall schrieb der Cron also gar nichts — ein toter Cron hinterließe dieselbe Spur wie ein ruhiger. Jeder Lauf schreibt jetzt eine `cron-run`-Zeile nach `coffee_push_diag` (PR #41), bewusst dort statt in einer neuen Spalte: keine Schema- und keine Rechteänderung, und eine Historie statt nur des letzten Zeitpunkts. **PR #40 und #41 gemerged** (`d52d129b`, `d099d22c`) — der Merge löst bei diesem Projekt ein automatisches Production-Deployment aus, danach die ganze Kette verifiziert; `master` und Produktion sind derselbe Stand. Cron-Registrierung geprüft: Vercel, Region `iad1`, `disabledAt: null`, Zielhost folgt dem aktuellen Production-Deployment; 06:17 UTC = 08:17 Wien (Sommer) bzw. 07:17 (Winter). 26 API- + 10 Frontend-Tests |
| 2026-08-24 | **Cron-Beweis erbracht**: planmäßiger Lauf 06:21 UTC (`tier=ok leer=2026-12-21`) — die Kette Zeitplan → Funktion → DB → Entscheidung läuft von selbst. Sichtbar nur durch das Lebenszeichen aus PR #41: bei `tier=ok` wird nichts gesendet und daher auch kein `notified_on` geschrieben. **Sicherheitslücke geschlossen (PR #42):** `if (!isCron && !isTest) return 401` ließ den Testschalter **allein** als Zugang durchgehen — da `/api/push/check-reorder` öffentlich erreichbar ist, konnte jeder mit der Adresse eine Push-Nachricht auf das Gerät des Nutzers auslösen. Aufgefallen in den eigenen Diagnosedaten (ein Lauf 09:01 UTC mit `user_agent: test=1`, den kein Zeitplan erklärt). Jetzt wird der Schlüssel **immer** verlangt; `?test=1` ändert nur noch das Verhalten, nicht den Zugang. 27 API-Tests. Bestellung 74353 **versendet** am 24.08. 11:25 (Sendung 506647-001, Post.at). Betriebsbefund: eine `until`-Warteschleife gegen den eigenen Endpunkt löst **Vercels Attack Challenge Mode** aus und sperrt die eigene IP für 403 aus — Cron (Vercel-intern) und Endgeräte bleiben unberührt |
| 2026-09-04 | **Die zwei neuen Sorten waren nie eingetragen — und der Weg dorthin war zu.** Kontrolle ergab: `coffees` hatte 33 Zeilen, jüngster Eintrag **ReKico Flor vom 26.06.**; Haiti Azeglio und Hausbrandt Venezia aus Bestellung 74353 (23.08.) fehlten. **Alle vier Lesewege von dieser Maschine waren zu:** Supabase-MCP `401 AUTH_HEADER_REJECTED`, `pass supabase/mcp-pat` und `supabase/access-token` beide abgelaufen (401), der Anon-Key aus dem Prod-Bundle seit der JWT-Brücke `401 permission denied`, und die Rolle `coffee_push` erreicht nur `coffee_stock`/`coffee_push_subscriptions`/`coffee_push_diag`. `vercel env pull` liefert für `SUPABASE_JWT_SECRET_COFFEE` und den Anon-Key **leere Werte** — als sensitiv markiert, nicht rücklesbar. **Gelöst über das Projekt-DB-Passwort** aus dem 1Password-Item „Supabase - CoffeeTracker - DB" (`dbPass`) als `postgres.hrgdmjzouzntloyngcrh` am Pooler — das läuft nicht ab, im Gegensatz zu jeder PAT. **Produktdaten von den Seiten gelesen, nicht abgeleitet:** beide Sorten führen „Arabica/Robusta Anteil: 50/50" explizit, Azeglio dunkel / Venezia mittel, Art-Nr SW10449 und VT1270 bestätigt. Einzig `origin` (`SA,AS`) ist abgeleitet, es steht auf keiner der Seiten. Borbone Blu bekam **keine** neue Zeile — Wiederkauf. **Bilder lokal ausgeliefert (PR #44):** 800×800 WebP wie das Borbone-Bild. Verifikationsfalle dabei: der SPA-Rewrite liefert für jede fehlende Datei `index.html` mit **Status 200**, ein `curl -f` mit Retry hätte ewig „erfolgreich" ins Leere gegriffen — das Kriterium ist `content_type: image/webp`, nicht der Statuscode. **`coffee_purchases` (PR #45):** ein Kauf = eine Zeile, weil `coffees` auf `roaster`+`description` dedupliziert und ein Wiederkauf dort keine Spur hinterlässt (derselbe Grund, aus dem `coffee_stock` eine eigene Tabelle ist). Verworfene Alternative: den Verlauf als Text in `comment` sammeln — funktioniert heute, hört beim nächsten Formatwechsel still auf, ist weder sortier- noch summierbar. Rechte spiegeln `coffees` exakt, live gegengeprüft mit `anon → 401 permission denied` (kein `200 []`, das wäre RLS ohne `revoke`). Backfill: 4 Bestellungen, 7 Positionen, **8 Beutel, 165,22 €**, Borbone Blu sichtbar **4×**. UI: Statistik-Kachel, `4× gekauft`-Badge nur bei echten Wiederkäufen, volle Historie in der Detailansicht. `summarizePurchases` als reine Funktion herausgezogen, 8 Tests gegen die zwei naheliegenden Rechenfehler (Positionen statt Beutel zählen: 3 statt 4; Einzelpreise summieren statt Menge × Preis: 51,97 statt 69,96). 18 Frontend- + 27 API-Tests. **Bestand nachgeführt:** Venezia am 04.09. angebrochen ⇒ `opened_at = 2026-09-04`, `bags = 4` ⇒ leer am **2026-12-24** (vorher 05.08./5 Beutel ⇒ 21.12., eine Vorwegnahme der Lieferung). **Erstverkostung Venezia bei Mahlgrad 30 (~4 bar):** Crema ordentlich, Körper dünn, nicht zu stark ⇒ **Unterextraktion, kein Geschmacksurteil** — Bewertung bewusst auf `T0 C0` gelassen, eine Note dafür bewertete die Mühle und zöge Avg Rating und Value Score dauerhaft nach unten. Befund für die `preparationnotes`: **bei 50/50 Robusta taugt die Crema nicht als Extraktionsanzeige**, der Körper schon — Robusta liefert Crema auch bei zu kurzer Extraktion, wer auf die Crema schaut liest „passt schon". Zielrichtung 26 → 24 (aus zwei Punkten abgeleitet: Flor 23 ≈ 9 bar, Venezia 30 ≈ 4 bar ⇒ ~0,7 bar/Stufe, andere Bohne und in Wahrheit keine Gerade). **`/add-coffee` überarbeitet:** Schritt 1-bis prüft zuerst auf Wiederkauf (dann *nur* eine Kaufzeile, `coffees.price` bleibt der Erstkaufpreis), Schritt 4b baut die Kaufzeile, Schritt 6 dokumentiert den DB-Passwort-Fallback für den Fall der toten PAT |
